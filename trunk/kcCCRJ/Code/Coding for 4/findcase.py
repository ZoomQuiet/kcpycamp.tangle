...
@license: kcCCRJ
@version：1.0
@tag：从一行字符串中查找关键字case，若找到则返回紧接case的下一个文位置，否则返回-1

@param CodeLine：需检测的字符串 
@type CodeLine: 字符串
@param Start: 从该行字符串的第几个位置开始检测
@type Start: 整型
@return: 若找到则返回case的e所在位置，否则返回-1
@rtype i: 整型
"""
...
#    global k
    ...
    while i < len(CodeLine) - 4:
        ...
        if szTemp == 'case':
            if i+4 < len(CodeLine):#检测case是否某个变量的前缀，若是则found = 1并继续循环
                if 'a' <= CodeLine[i+4] <= 'z':
                    found = 1
                    continue
                if 'A' <= CodeLine[i+4] <= 'Z':
