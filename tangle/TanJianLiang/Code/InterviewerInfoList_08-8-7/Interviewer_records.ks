...
db=PyDbLite.Base('Interviewer_records.pdl')
db.create("Name","ID",mode="open")

def index():
    if len(db):
        Include("../Interviewer_recordsHeader.pih",title="My record collection")
        print '<h1>InterviewerInfoList</h1>'
    ...
    print '<p><a href="editRecord?recordId=-1">Add New Interviewer</a>'
    print '</body>\n</html>'
...
def editRecord(recordId):
    ...
    if recordId>-1:
        ...
        Include("../Interviewer_recordsHeader.pih",title="Editing record %s" %recordId)
        print "<h1>Editing a record</h1>"
    else:
        name,id='',''
        Include("../Interviewer_recordsHeader.pih",title="New Interviewer")
        print "<h1>New Interviewer</h1>"
