...
db=PyDbLite.Base('Assistant_records.pdl')
db.create("Name","ID",mode="open")

def index():
    if len(db):
        Include("../Assistant_recordsHeader.pih",title="My record collection")
        print '<h1>AssistantInfoList</h1>'
    ...
    print '<p><a href="editRecord?recordId=-1">Add New Assistant</a>'
    print '</body>\n</html>'
...
def editRecord(recordId):
    ...
    if recordId>-1:
        ...
        Include("../Assistant_recordsHeader.pih",title="Editing record %s" %recordId)
        print "<h1>Editing a record</h1>"
    else:
        name,id='',''
        Include("../Assistant_recordsHeader.pih",title="New Assistant")
        print "<h1>New Assistant</h1>"
